...
General Audit Manual의 표본 크기**(Size)**를 결정하고 표본의 선택**(Selection)** 방법에 대하여 정리  

각 Sampling 방법에 따라서 표본 크기를 정하는 Guide Line이 있으며 표본의 선택 방법은 위험의 평가와 감사 대상 항목의 특성에 따라서 적절하게 선택 적용
...
감사인은 모집단을 정의한 다음, 해당 모집단이 완전하고 감사 목적에 적합한지 평가  

모집단을 계층화 (stratification) 할지 여부는, 식별된 위험을 고려하여 전기의 테스트 결과를 통해 결정  

모집단내 항목들의 성격이 동질적이지 않아 계층화가 이루어져아하는 경우, 이러한 특성에 의해 왜곡표시의 가능성이 변동할 수 있음(예를 들어, 모집단 내의 특정 항목에는 상당한 위험이 있고 다른 항목은 그렇지 않을 경우, 복잡성이나 기타 특성에 따라 왜곡 표시되기 쉬운 항목이 있을 경우)  

감사인은 샘플링 단위를 결정할 시 다음의 factor들을 고려
...
감사인은 샘플링 위험을 수용 가능한 낮은 수준으로 줄이기에 충분한 샘플 크기를 결정

실질적 세부 내역 테스트를 위한 표본 크기를 결정할 시, 감사인은 다음 변수를 설정
...
주요 표본 추출 방법은 Systematic selection(MUS), Random, Haphazard 가 있음
...
또한, 감사인은 모집단의 계층화 (stratification) 를 통해, 각 계층 (stratum) 금액에 비례한 필요한 표본 크기를 추출하여, 표본이 모집단을 대표할 수 있는 가능성을 증가시킬 수 있음
...
.bg-washed-green.b--dark-green.ba.bw2.br3.shadow-5.ph4.mt5[
RSM KR Comment : Sample 에서 Test 를 수행 후 왜곡표시가 발견 되는 경우에 해당 항목에 대해서만 수정 또는 미수정을 고려하는 것은 올바른 방법이 아니며 Sample 대상에서 왜곡표시 발견 후에 이 항목이 전체 표본 (Population)에 끼치는 영향을 고려 해야 함. 전체 모집단 대비 추정오류를 계산하여 이 금액이 감사인이 허용할 만한 왜곡표시 금액인지 평가하는 과정을 거쳐야 함. 
]
...
감사인은 표본의 대상이 되는 특정 내부 통제 정책 및 절차를 식별, 문서화하고, 모집단을 결정함.  또한, 감사 증거를 수집하기 위한 테스트 절차를 설계한 후, 예상되는 편차를 정의해야함
...
감사인은 테스트 중인 통제에 비추어 샘플링 단위를 정의해야 합니다. 예를 들어, 구매에 대한 Accuracy를 테스트할 때, 회사가 송금 전에 바우쳐를 승인하는 통제인 경우, 감사인은 샘플링 단위를 다음와 같이 정의할 수 있음
...
감사인은 샘플링 위험 (sampling risk)을 허용 가능한 낮은 수준으로 줄이기에 충분한 샘플 크기를 결정해야함  
감사인의 수용 가능한 위험이 낮을수록 표본 크기가 커짐
...
발생 횟수는 대상 기간내 통제 활동이 발생한 횟수 이고, 통제 활동은 주기적으로(예: 매주, 매월) 작동하거나 특정 거래 또는 이벤트가 발생할 때마다 발생할 수 있음
...
모집단이 250회 이상으로 발생하는 경우, 표본 크기는 주장 (Assertion) 과 관련된 상당한 위험이 있는지 여부와 모집단의 발생 횟수에 의해서만 영향을 받음
...
통제 테스트 시 표본을 선택하는 주요 방법은 Random selection, Systematic selection 및 Haphazard selection이 있고, 가장 연관성 있는 방법은 Random 및 Haphazard selection

### Using work of others

중대한 위험이 없는 경우, 감사인은 다른 사람(내부통제부서, 내부감사팀 또는 내부회계관리담당 및 외주로 수행한 내부통제운영 등)이 수행한 작업의 재수행을 수행할 수 있으며, 통제에 대한 독립적인 테스트를 수행하지 않을 수 있음  
...
.bg-washed-green.b--dark-green.ba.bw2.br3.shadow-5.ph4.mt5[
RSM KR Comment : Attribute sampling (속성 샘플링)은 입증 감사 시 (Not 내부통제 Test) 관련 속성의 발생 (Occurrence) 사실의 존재 유무를 Test하는 접근법이며 Yes or No로 구분되는 경우 사용하는 접근법.  
Attribute sampling 입증 감사 자체로 충분한 감사 증거를 제공하지 않으며, 이에 따라 감사인의 주장을 확인하기 위해 입증적 분석 절차 또는 내부 통제에서 낮은 수준의 Assurance를 가졌다는 확신 후 진행할 수 있는 내용. (Low level of assurance from a substantive analytical procedure or perform tests of controls related to the assertions being tested)
]
...
따라서, 테스트 중에 단일 편차가 식별되면, 감사인은 모집단 내 속성의 존재에 대해 충분한 증거를 수집하지 못한 것으로 결론 - 결과적으로, 감사인이 예상 표본 편차 비율이 0이 아닐 것이라고 생각하는 경우 감사인은 대체 샘플링 절차를 수행해야함

감사인은 다음의 테이블에 따라 샘플 크기를 결정해야함
...
감사인이 속성 샘플에서 오류를 발견하게 될 경우, 감사인은 샘플 결과가 주장에 대해 해당 속성의 존재에 대한 충분한 증거를 제공하지 않는다는 결론을 도출 할 수 있음
...
감사인은 모집단 값의 백분율로 선택된 항목의 총 가치가 다음 테이블에 나열된 적용 범위 (Scoping Coverage) 에 도달하거나 초과할 때까지 값 내림차순으로 모집단에서 특정 항목을 선택해야함
...
감사인은 왜곡표시의 원인 및 성격을 이해하고, 중요한 왜곡표시 위험에 대한 기존 평가를 재고해야함  
