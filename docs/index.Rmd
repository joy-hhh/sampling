...
통계적 샘플링 : 다음과 같은 특성을 가진 샘플링 접근 방식

* 샘플 항목의 무작위 선택, 그리고
* 샘플링 위험 측정을 포함하여, 샘플 결과를 평가다음 위해 확률 이론이 사용됨. 

상기 특성이 없는 샘플링 방식을 비통계적 샘플링임. 
...
감사인은 샘플링 위험을 수용 가능한 낮은 수준으로 줄이기에 충분한 샘플 크기를 결정해야함. 
...
비통계적 샘플링 시, 감사인은 먼저 모집단에서 금액이 큰 모든 항목을 테스트 해야함. 
이때, 금액이 큰 항목은 샘플링 간격 (Interval)보다 큰 항목이며, 이를 high value treshold라고도 함. 
샘플링 간격은 다음의 공식을 통해서 계산할 수 있음

$$ Sampling \ Interval = (Tolerable \ Misstatement \ – \ Expected \ Misstatement) \over Assurance \ Factor $$

금액이 큰 항목은 high value treshold 미만의 항목과는 별도로 테스트 되어야함. 이때, 감사인은 큰 금액의 항목에서 발견된 왜곡 표시를 전체 모집단에 투영해서는 안됨 .
해당 금액이 큰 항목을 제외한 나머지 모집단에서 샘플링 및 고액 항목을 제외한 모집단에 애한  왜곡 표시 projection이 이루어 져야함. 

---
.bg-washed-green.b--dark-green.ba.bw2.br3.shadow-5.ph4.mt5[
RSM KR Comment : High value items (Sampe Interval 초과 item)은 별도로 모든 Item에 대해 Test 수행해야 함. Non-high value items (Sampling interval 이하의 item으로 구성 된)에 대해서 Sampling을 수행 및 Test를 하며 이에 대해 Error evaluation을 해야 하며 추정오류를 산출할 수 있음. BP60 ~ BP70 참고								
]
...
감사인은 주장(assertion)에 대한 중요한 왜곡표시의 위험과 직접 연관된 통제를 식별 하여, 어느 통제를 선택할 것인지, 그리고 얼마나 많은 통제를 선택해야 하는지 결정해야함.
단일 통제를 통해 개별 주장에 대한 중요한 왜곡표시의 위험을 충분히 다룰 수 있지만, 종종 여러 통제 활동의 조합에 대한 테스트가 요구되는 경우가 있음.
감사인은 주어진 주장에 대한 모든 통제를 테스트할 필요는 없으나, 감사인은 통제가 존재하는 프로세스 및 시스템의 맥락 상 관련 통제의 효율성을 고려해야함.

단일 통제의 효과성이 다른 통제의 효과성과 밀접한 연관이 있는 경우가 있음.
예를 들어, 경영진의 판매 보고서 주간 검토에 대한 효과성 테스트 시, 감사인은 판매 보고서에 사용된 데이터의 정확성과 완전성에 대한 통제 효과성을 테스트함으로써, IT 통제에 대한 신뢰성을 얻을 수 있음. 
...
1. 하나의 일반 통제를 통해 여러 어플리케이션에 대한 통제가 이루어지는 경우, 감사인의 모집단은 "1"이 될 수 있음. 예를 들어, 통제 테스트 범위 내의 어플리케이션들이 단일 배치 처리 시스템을 통해 추적이 가능한 경우, 야간 배치 프로세스의 단일 모집단을 샘플링하는 것이 적절할 수 있음. 
2. 여러 IT 어플리케이션에 대해 유사한 통제가 존재하는 경우, 감사인은 각 에플리케이션에 대해 샘플링을 진행해야하고, 이때 모집단은 각 어플리케이션 별 1임. 예를 들어, 여러 IT어플리케이션에 대한 사용자 접근 통제를 테스트할 때, 각 응용 프로그램에 대해 샘플링을 해야함.
...
1. 테스트 중인 특정 속성, 특성, 트랜잭션 프로세스의 특성 등을 식별하고 문서화
2. 특정 속성, 특성 또는 트랜잭션 프로세스의 특성에 대한 증거를 포함하는 샘플링 단위를 식별.
3. 감사 절차의 목적을 고려하여 예외 사항에 해당하는 요소를 결정.

속성 테스트를 위한 모집단은, 동일한 프로세스 및 내부 통제 활동의 대상으로서, 동종 (Homogeneous) 항목으로 구성되어야함.
계정 잔액 또는 거래 종류 (Class of transaction)의 속성이 고유하게 다를 수 있고, 이 경우 해당 두개 이상의 다른 모집단으로 분리해야함.
...
감사인이 충분한 증거를 얻기 위해 다른 형식의 세부 테스트(예: 비통계적 샘플) 기법을 사용다음로 결정한 경우, 감사인은 속성 샘플에서 이미 테스트된 항목을 새로운 샘플에 포함하고, 나머지 테스트되지 않은 항목에서 추가 항목을 선택해야함.  
...
적용범위 외 모집단의 총 금액이 전체 중요성 또는 특정 중요성 금액을 초과하는 경우

1. 감사인이 수행한 분석적 절차가 효과적으로 수행되었을 경우 , 추가 테스트는 필요 없음.
2. 감사인이 수행한 분석적 절차가 효과으로 수행되지 않았을 경우, audit sampling 등을 통하여 추가 감사 증거를 수집해야함.
