...
모집단내 항목들의 성격이 동질적이지 않아 계층화가 이루어져야하는 경우, 이러한 특성에 의해 왜곡표시의 가능성이 변동할 수 있음(예를 들어, 모집단 내의 특정 항목에는 상당한 위험이 있고 다른 항목은 그렇지 않을 경우, 복잡성이나 기타 특성에 따라 왜곡 표시되기 쉬운 항목이 있을 경우)  
...
해당 금액이 큰 항목을 제외한 나머지 모집단에서 샘플링 및 고액 항목을 제외한 모집단에 대한  왜곡 표시 projection이 이루어져야함
...
이 샘플링 방식은 중대한 위험(Significant Risk)이 있는지 여부에 관계없이 사용이 가능함 (즉, 중대한 위험이 있는 경우와 중대한 위험이 없는 경우 모두 사용이 가능함)
...
RSM KR Comment : PM의 3배수 이하인 경우에 사용 가능하며, Sample size는 위의 방법으로 결정 가능하나 표본추출 방법은 여전히 Haphazard, Systemetic, Random 등의 방법을 사용함. 하지만, 해당 결과로서 Error가 발견 된다면, 감사인은 추가적인 절차 (추가 Sample selection 등)를 취하고 Sample evaluation 을 수행해야 함
]
